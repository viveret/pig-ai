...
namespace ScriptAI {
    namespace Sql {        
        class LexiconQueryLeastFrequentPagedCmd: public SqlCommand<paged_position, LexiconFeatureModel, std::vector<LexiconFeatureModel>> {
            public:
            LexiconQueryLeastFrequentPagedCmd(sqlite3* sqlContext): SqlCommand("SELECT occurrences, freq, label FROM Lexicon ORDER BY occurrences ASC LIMIT @start,@count", sqlContext) { }
